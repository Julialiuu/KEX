...
"""
class Movie:
    def __init__(self, title, revenue, imdb_rating, wiki_sv, wiki_eng, production_budget, sf_rating, mpaa_rating,director,writer,actor):
        self.title=title
...
def create_objects(file_name):
    ...
    for r in file_name.sheets():
        for row in range(1,r.nrows):
            movie_object=Movie(r.cell(row,0).value,r.cell(row,1).value,r.cell(row,7).value,r.cell(row,10).value,r.cell(row,11).value,r.cell(row,5).value,r.cell(row,6).value,r.cell(row,4).value,r.cell(row,12).value,r.cell(row,13).value,r.cell(row,14).value)
            movie_list.append(movie_object)
            print(movie_object.title, movie_object.revenue, movie_object.imdb_rating, movie_object.wiki_sv,movie_object.wiki_eng,movie_object.production_budget,movie_object.sf_rating,movie_object.mpaa_rating, movie_object.director, movie_object.writer, movie_object.actor)
    return movie_list
...
def print_csv(movie_list):
    ...
    for element in movie_list:
        if element.writer != 0:
            res.append([element.revenue,element.writer])
    my_df=pd.DataFrame(res)
    my_df.to_csv('writer.csv',index=False,header=False)
    print(my_df)
    """
...
def cross_vali():
    df=pd.read_csv("wiki_eng_sf_rating
    .csv")
